...
// network
export async function displayNetwork(label = "Symptom", name = "肩背痛") {
  let networkChart = getEchartInstance("network");
  networkChart.showLoading();
  let networkOption = await renderNetwork(label, name);
  ...
  networkChart.on("click", (params) => {
    ...
    let name = data.name;
    let label = data.des.split("::")[0];
    renderNetwork(label, name).then((option) =>
        networkChart.setOption(option)
    );
  });
}

export async function renderNetwork(label = "Symptom", name = "肩背痛") {
...
// main for rust
export async function main(label = "Symptom", name = "肩背痛") {
  await displayNetwork(label, name);
  await displayStats();
}
