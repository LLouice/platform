...
class Loss(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def _get_loss_multiplier_for_tensor(self, tensor, losses_mask):
        ...
        return tf.cast(tf.reshape(losses_mask, loss_multiplier_shape),
                       tf.float32)
...
class WeightedSoftmaxClassificationAgainstLogitsLoss(Loss):
  ...
  """
    ...
    def _scale_and_softmax_logits(self, logits):
        ...
        scaled_logits = tf.divide(logits,
                                  self._logit_scale,
                                  name='scale_logits')
...
class BootstrappedSigmoidClassificationLoss(Loss):
  ...
  """
    ...
    """
        ...
        per_entry_cross_ent = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=bootstrap_target_tensor,
    ...
    """
        ...
        for ind, detection_boxlist in enumerate(decoded_boxlist_list):
            ...
            mined_location_losses.append(
                tf.reduce_sum(tf.gather(location_losses[ind],
                                        selected_indices)))
...
def symmetric_cross_entropy(alpha, beta, A=1e-4):
...
def symmetric_cross_entropy_stable(alpha, beta, A=1e-4):
    def loss(y_true, logit):
        ...
        loss_ce = alpha * tf.reduce_mean(
            -tf.reduce_sum(y_true_1 * tf.math.log_sigmoid(logit_1), axis=-1))
        loss_ce = tf.identity(loss_ce, name="loss_ce")
        loss_rce = beta * tf.reduce_mean(
            -tf.reduce_sum(tf.sigmoid(logit_2) * tf.log(y_true_2), axis=-1))
        loss_rce = tf.identity(loss_rce, name="loss_rce")
        return loss_ce, loss_rce
